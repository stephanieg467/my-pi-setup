/**
 * Takeover UI for Claude agents (ported from v1, rendering from the synchronous
 * SubagentReadModel instead of live pi sessions):
 * - ClaudeAgentDashboard: full popup (overlay) listing all Claude agents.
 * - TakeoverView: full interactive view of one Claude agent with an input line
...
export async function openSubagentPicker(
  ...
  while (true) {
    if (view.size() === 0) {
      ctx.ui.notify("No Claude agents", "info");
      return;
    }

    const picked = await ctx.ui.custom<string | null>(
      (tui, theme, keybindings, done) =>
        new ClaudeAgentDashboard(tui, theme, keybindings, view, selection, done),
      {
        overlay: true,
        overlayOptions: { anchor: "center", width: "100%", maxHeight: "100%" },
      },
...
class ClaudeAgentDashboard implements Component {
  private tui: TUI;
  private theme: Theme;
  ...
  render(width: number): string[] {
    ...
    const headerLeft = theme.fg("accent", theme.bold("Claude Agents"));
...
class TakeoverView implements Component, Focusable {
  ...
  /** Dispatch takeover input, retaining it when the backend rejects the send. */
  private async submitInput(value: string): Promise<void> {
    ...
    this.tui.requestRender();
    const result = await this.view.requestSend(this.id, text);
    if (result.ok) {
      this.input.setValue("");
      this.scrollOffset = 0;
    } else {
  ...
  handleInput(data: string): void {
    ...
    if (!this.sending) this.input.handleInput(data);
    this.tui.requestRender();
  }

  private viewportHeight(): number {
    const rows = this.tui.terminal.rows || 30;
    const transcriptPathRows = this.snap()?.meta.sessionFilePath ? 1 : 0;
    // The complete view renders viewport + 7 chrome rows, plus transcript
    // metadata when available. Keep the overlay at terminal rows - 1.
    return Math.max(6, rows - 8 - transcriptPathRows);
  }
  ...
  render(width: number): string[] {
    ...
    if (snap.meta.sessionFilePath) {
      lines.push(
        truncateToWidth(
          theme.fg("dim", `transcript: ${snap.meta.sessionFilePath}`),
          width,
        ),
      );
    }
    ...
    if (this.sendError) {
      body.push(
        truncateToWidth(
          theme.fg("error", `send failed (input retained): ${this.sendError}`),
          width,
        ),
      );
    }
