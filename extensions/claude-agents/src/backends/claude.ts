      ...
      try: () =>
        query({
          ...
          options: {
            ...
            // already chose to launch an autonomous Claude agent, so let it use
            ...
            // and concurrency cap rather than Claude Code's native child agents.
    ...
    return {
      ...
      send: (text) =>
        Effect.suspend((): Effect.Effect<void, SendError> => {
          if (state.closed) {
            return new SendError({ message: "Claude agent session is closed." });
          }
          return submit(text)
            ? Effect.void
            : new SendError({ message: "Claude agent session is closed." });
        }),
