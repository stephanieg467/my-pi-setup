 ...
 * `SubagentSnapshot`. Closing a Claude agent's scope kills the underlying
...
export interface SubagentReadModel {
  ...
  /** Steer/continue a subagent and report the bounded dispatch outcome. */
  requestSend(id: string, text: string): Promise<SendRequestResult>;
        ...
        (): Effect.Effect<void, SpawnError | ConcurrencyLimitError> => {
          if (disposed) {
            return new SpawnError({
              message: "Claude agent manager is shutting down.",
            });
          }
          if (runningCount() + reserved >= MAX_RUNNING) {
            return new ConcurrencyLimitError({
              message: `Max ${MAX_RUNNING} Claude agents can run concurrently. Wait for one to finish before spawning another.`,
            });
          }
          reserved++;
          return Effect.void;
      ...
      const doSpawn = Effect.gen(function* () {
        ...
        if (disposed) {
          ...
          return yield* new SpawnError({
            message: "Claude agent manager shut down while spawning.",
          });
        }

        const origin = task.origin ?? "model";
        const id = `claude-${++modelCounter}`;
        ...
        // Claude agent still looks running, the backend died out from under us.
    ...
    Effect.suspend((): Effect.Effect<void, SendError> => {
      ...
      if (!entry || disposed) {
        return new SendError({
          message: `Claude agent "${id}" is no longer tracked.`,
        });
      }
      // Restarting a settled Claude agent occupies a running slot again, so it
      // must respect the same cap as spawn. Steering an already-running one
      // does not consume additional capacity.
      if (entry.snapshot.status !== "running") {
        if (runningCount() + reserved >= MAX_RUNNING) {
          return new SendError({
            message: `Max ${MAX_RUNNING} Claude agents can run concurrently; restarting "${id}" would exceed that.`,
          });
        }
          ...
          send(id, text).pipe(
            Effect.match({
              onFailure: (error) =>
                resolve({
                  ok: false,
                  error: bounded(error.message.replace(/\s+/g, " ")),
                }),
