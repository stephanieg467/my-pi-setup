/**
 * Claude agent domain model.
 *
 * Everything downstream of a backend (manager, tools, UI) speaks only these
 * types. The backend translates Claude Agent SDK messages into the normalized
 * `SubagentEvent` union.
 */

import { Data } from "effect";

export const BACKEND_NAMES = ["claude"] as const;
export type BackendName = (typeof BACKEND_NAMES)[number];

/** Who initiated the session. User asides stay out of model-facing tooling. */
export type SubagentOrigin = "model";

/**
 * Shared reasoning-effort scale mapped to Claude thinking-token budgets.
 * Omitted means the Claude Code default.
 */
...
export interface SpawnTask {
  ...
  readonly cwd: string;
  /**
   * Claude model alias. Omitted uses the Claude Code default.
   */
  readonly model?: string;
  /** Shared effort scale mapped to Claude thinking tokens. */
...
export interface SubagentMeta {
  ...
  /** Display label reported by Claude Code. */
  readonly modelLabel?: string;
  /** Context window capacity for utilization display, when known. */
  readonly contextWindow?: number;
  /** Claude projects JSONL transcript path. */
...
/**
 * The manager folds `SubagentEvent`s into one snapshot per Claude agent. This is
...
export interface SubagentSnapshot {
  ...
  /** Count of finalized assistant messages (for claude_agent_check). */
  readonly turns: number;
}

/** Return final text, or the live streaming buffer while a run is active. */
...
/** Format elapsed runtime for a Claude agent snapshot. */
