...
This is the shareable part of my [Pi](https://pi.dev/) workspace: a dark GitHub-inspired theme, focused prompts and agent roles, and a Claude Agent SDK extension. It is intentionally opinionated and personal—not a universal starter kit—but I hope it offers useful ideas and building blocks.
...
The root `package.json` makes this a proper Pi git package. Installing it loads the extension, Claude Agent SDK skill, all prompts, and the theme. The files in `agents/`, `AGENTS.md`, and `settings.example.json` are references to merge deliberately; package installation does not replace your existing global configuration.
