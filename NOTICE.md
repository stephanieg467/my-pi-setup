# Third-party notices

This repository curates Stephanie's live Pi configuration. Some included resources have upstream origins and are not solely Stephanie-authored.

## Ben Davis's `my-pi-setup`

The following resources were adapted from [davis7dotsh/my-pi-setup](https://github.com/davis7dotsh/my-pi-setup):

- `themes/github-dark-default.json`
- `extensions/claude-agents/`, derived from the upstream subagents extension and adapted to use the Claude Agent SDK

Ben's repository did not include a license when this setup was prepared. These upstream-derived portions are not covered by Stephanie's MIT grant below. Preserve this notice and confirm the upstream terms before redistributing them outside GitHub's repository-forking features.

## Archon

`skills/archon/` comes from [coleam00/Archon](https://github.com/coleam00/Archon) and is distributed under its bundled MIT license:

> Copyright (c) 2025-2026 Cole Medin

See [`skills/archon/LICENSE`](skills/archon/LICENSE).
