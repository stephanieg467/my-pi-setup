...
Interactive upstream setup guide. It assumes Claude Code's `AskUserQuestion` tool and must be adapted when another agent harness is in use.

**IMPORTANT — Input safety**:
- Use a structured question tool only for multiple-choice decisions.
- Ask for non-sensitive paths, URLs, and usernames in plain text.
- Never request tokens or other credentials in chat. Have the user enter them through Archon's separate-terminal setup flow.
...
**If `git` is not installed**: Ask for explicit approval before running the appropriate platform installer:
...
**If `bun` is not installed**: Direct the user to review and follow the official installation instructions at https://bun.sh/docs/installation in a separate terminal. Do not pipe a remote script into a shell on the user's behalf.

The installer adds Bun to the shell config, but it may not take effect until a new shell starts. For the rest of this setup session, use the full path `~/.bun/bin/bun` instead of just `bun` when applicable.
...
On Windows, follow the official PowerShell instructions in the separate terminal. The installer should update `PATH` for that session; verify with `bun --version`.
...
This snapshot does not assume the current directory is Archon's source repository. Ask for or locate the user's Archon checkout and store it as `<archon-repo>`. Stop if no valid checkout is available.
...
4. Check Claude is installed with `which claude`; if authentication is needed, start `claude` and run `/login` interactively
